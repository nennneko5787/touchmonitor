...
//!  2. H.264-encodes it with OpenH264 (Baseline profile).
...
fn main() {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--bind" => bind = args.next().unwrap_or_default(),
            "--port" | "-p" => {
    ...
    if let Err(e) = stream::run_server(&bind, port, monitor_index, fps, bitrate_kbps) {
...
fn print_help() {
    println!(
        "touchmonitor-server\n\n\
         ...
         options:\n\
         \x20 --bind <ip>          IP to listen on (default: 0.0.0.0)\n\
